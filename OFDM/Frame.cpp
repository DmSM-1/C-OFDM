...
FFT_FORM::FFT_FORM(int fft_size,int num_data_subc,int num_pilot_subc, int num_symb, double pilot_ampl)
    :   fft_size(fft_size),
...
complex_vector& FFT_FORM::read(){
    ...
    phys_pilot_ampl /= pilot.size()*pilot_ampl;
...
void PREAMBLE_FORM::set(complex_double* buf_ptr){
    ...
    write(preamble);
    memcpy(ofdm_preamble.data(), output[0], ofdm_preamble.size()*sizeof(complex_double));
    mod_preamble = Mod.mod(preamble);

    double norm = 0.0;
    for (int i = 0; i < pr_sin_len; i++){
        conjected_sinh_part[i] = std::conj(ofdm_preamble[i]);
